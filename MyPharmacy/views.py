...
class MedicalType_view(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = Medical_Type_Selializers(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class MedicalType_details(APIView):
    def get_object(self, pk):
        try:
            medical_type = Medicaltype.objects.get(id=pk)
            
        except medical_type.DoesNotExist:
            return Response(status=status.HTTP_400_BAD_REQUEST)


    def get(self, request, pk=None):
        medecine_type = self.get_object(pk)
        serializer = Medical_Type_Selializers(medecine_type)
